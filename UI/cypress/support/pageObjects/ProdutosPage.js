class ProdutosPage {
    ...
    buscarProduto(nomeProduto) {
        ...
        cy.get(".product_title", { timeout: 10000 }).should("be.visible");
    }
    ...
    definirQuantidade(quantidade) {
        cy.get('input[name="quantity"]', { timeout: 7000 }).clear().type(quantidade);
    }
    ...
    verificarListaDeProdutosVisivel() {
        cy.get('.product-block', { timeout: 7000 }).should('exist').and('be.visible');
    }

    verificarTituloProduto(nomeProduto) {
        cy.get(".product_title", { timeout: 10000 }).should("contain.text", nomeProduto);
    }

    verificarMensagemProdutoNaoEncontrado(nomeBusca) {
        cy.get('.page-title', {timeout: 7000}).should('contain.text', `Resultados da pesquisa por: “${nomeBusca}”`);
    ...
    verificarMensagemSucessoAdicionar(nomeProduto) {
        ...
        cy.get('.woocommerce-message > .button.wc-forward', { timeout: 7000 }).should('be.visible');
    }
    ...
    verificarProdutoNoCarrinhoComVariaçoes(nomeProduto, tamanhoEsperado, corEsperada) {
    ...
    verificarMensagemEscolhaOpcaoProduto() {
        cy.get(".woocommerce-error", {timeout: 7000}) 
            .should('be.visible')
            .invoke('text') 
            .then(text => {
                const cleanedText = text.replace(/\s+/g, ' ').trim();
                expect(cleanedText).to.contain("Escolha as opções de produtos visitando Aero Daily Fitness Tee"); 
            });
    }

    verificarStatusForaDeEstoque() {
        cy.get('.stock', { timeout: 7000 }) 
            .should('be.visible')
